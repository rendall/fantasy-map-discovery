...
export const routeHandler: (
  ...
  for (let i = 0; i < locationIds.length - 1; i++) {
    ...
    if (!startCell || !endCell) {
      console.error(
        "One or both of the provided cell indices are not found in the map data."
      );
      process.exit(1);
    }

    const heightExponent = parseInt(data.settings.heightExponent);
    const route = findRouteAStar(
      startCell,
      endCell,
      cells,
      data.biomesData.cost,
      heightExponent
    );
    if (route) {
      ...
      console.log(
        `No route found between locations ${locations[i]} and ${
          locations[i + 1]
        }.`
      );
      process.exit(1);
  ...
  if (finalRoute.length > 0) {
    console.log("Route:");
    finalRoute.forEach((cell, i, route) =>
      console.log(
        `Cell i: ${cell.i}, distance: ${addDistance(i, route)} ${
          data.settings.distanceUnit
        }, ${showBurg(cell.burg)} biome: ${
          data.biomesData.name[cell.biome]
        }, position: (${cell.p[0]}, ${cell.p[1]})`
      )
    );
  } else {
